...
只做冻结 VGGT 推理、360° Adapter、confidence filtering、gsplat Gaussian 优化、Unity 键鼠 Demo、定量评估和可复现 GitHub 项目。不训练 VGGT，不增加语义分割，不加入 OpenXR/头显、Quest standalone 或大型数据集训练。
...
3. 用户提供真实多视角图或多中心全景后，保持同一命令运行。真实场景数据尚未提供时，不声明真实场景泛化已经验证。
...
| 5 | Unity 键鼠 | Gaussian/Geometry/Confidence 切换、自由移动、缩放、点击检查、FPS 测量；必须实际在激活的 Editor 中验证后才能标记运行完成 |
| 6 | GitHub Demo | 说明、单命令入口、真实指标、预览图/视频、Unity 项目、复现命令 |
...
- 系统：VGGT 推理时间/显存，Gaussian 优化时间/显存、数量、文件大小；服务器 headless CUDA FPS 与 Unity 帧率分开记录。Unity CPU allocated memory 不冒充 GPU memory。
...
6. 没有真实数据、Unity 激活或 GitHub 发布失败时，把该项标为待完成，并具体记录依赖。其余阶段继续运行。
7. 完成上述范围后停止扩展功能。后续提升优先添加真实多中心全景验证，而不是新增语义/XR 模块。
