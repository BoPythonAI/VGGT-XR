...
with zipfile.ZipFile(out/'release/VGGT-XR-demo-assets.zip','w',zipfile.ZIP_DEFLATED) as archive:
    ...
    archive.writestr('README.txt','VGGT-XR actual gsplat models and videos. Videos are CUDA gsplat footage and form the final visual validation; Unity runtime validation is outside the current stop line. Optional Unity export code remains in the repository. Models are inference-derived from the original non-commercial VGGT-1B checkpoint: use for academic/non-commercial demonstration; upstream terms apply. Input RGB datasets and model weights are not included. Kitchen input source: facebookresearch/vggt examples/kitchen; panorama rooms include analytical synthetic scenes and a separately documented ZInD protocol. Original pipeline source is MIT. See reports for raw measurements and protocols.')
