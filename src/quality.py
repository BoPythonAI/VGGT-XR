...
def train_quality(xyz, rgb, images, extrinsics, intrinsics, output, *, steps=12000,
    ...
    scales=np.sqrt(np.mean(distances[:,1:]**2,axis=-1)).clip(scene_scale*1e-5,scene_scale*.1)
    ...
    rates=dict(means=1.6e-4*scene_scale,scales=.005,quats=.001,opacities=.05,sh0=.0025,shN=.0025/20)
    ...
    for step in range(steps):
        ...
        degree=min(sh_degree,step//max(250,steps//12))
        rendered,alpha,info=render(params,view,ks[i:i+1],w,h,return_info=True,sh_degree=degree,absgrad=densify,antialiased=antialiased,background=background)
        ...
        with torch.no_grad():
            params['scales'].clamp_(np.log(scene_scale*1e-6),np.log(scene_scale*2));params['opacities'].clamp_(-12,12)
        if len(params['means'])<64:raise RuntimeError('Densification pruned nearly all Gaussians')
        if step%500==0 or step==steps-1:
            row=dict(step=step+1,loss=float(loss.detach()),l1=float(l1.detach()),gaussian_count=len(params['means']),sh_degree=degree)
    ...
    result=dict(**config,gaussian_count=len(params['means']),scene_scale=scene_scale,train_seconds=seconds,peak_vram_gb=torch.cuda.max_memory_allocated()/2**30,history=history,opacity_reset_events=strategy.reset_events,gsplat='1.5.3',opacity_reset_fix='Corrected Python chained-comparison condition in local wrapper; package unchanged',pose_protocol='Training RGB only; first camera/group fixed; shared world SE(3) correction preserves panorama rig; bounded corrections' if pose_opt else 'Fixed training cameras',background=background)
